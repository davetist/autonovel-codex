...
class PipelineFeedbackLoopTests(unittest.TestCase):
    ...
    def test_drafting_appends_eval_canon_entries_for_accepted_chapter(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("- The archive uses a delayed public feed during council segments. *(ch01 eval)*", canon_text)
    ...
    def test_revision_auto_uses_existing_panel_for_surgical_pass_without_broad_adversarial(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (edit_logs / "reader_panel.json").write_text(json.dumps({
                "readers": {
                    "editor": {"missing_scene": "Chapter 23 needs the parent receipt scene."},
                    "reader": {"missing_scene": "Chapter 23 should show the parent receiving the private packet."},
